...
export default function MovieDetail({ params }: { params: { movieId: string } }) {
  ...
  const addSeat = (seat: SEAT) => {
    if (seat.status === SEAT_STATUS.AVAILABLE) {
      const foundIndex = selectedSeats?.findIndex((selected) => selected.row === seat.row && selected.no === seat.no);
      if (foundIndex > -1) {
          ...
          <div key={row} className="flex flex-row items-center gap-4">
            ...
            <div className="flex flex-row flex-1 items-center gap-4 justify-center py-4">
              {seats.map((seat) => {
                ...
                if (selectedSeats.find((selectedSeat) => selectedSeat.row === row && selectedSeat.no === seat.no)) {
                  color = "bg-green-500";
                }
                ...
                return (
                  <div
                    ...
                    className={`flex ${color} items-center hover:cursor-pointer justify-center border-[0.5px] rounded-sm w-[32px] h-[32px]`}
                  ></div>
                );
              })}
            </div>
